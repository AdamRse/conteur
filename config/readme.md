...
- Pour le faire, je définis la variable `PHP_LARAVEL_LATEST` ce JSON, et demmande de la rempalcer par `PHP_VERSION`, fourni par la bibliothèque laravel, dans `.PROJECT_TYPE.laravel.templates.Dockerfile.variables[]` :
```json
{
    "PROJECT_TYPE":{
        "laravel":{
...
- `PROJECT_TYPE` : Liste des bibliothèques disponibles
    - `PROJECT_TYPE`: Options relatives à la création d'un projet laravel
    ...
    - `laravel.templates` : Options relative à l'écriture depuis les templates définis dans `templates/laravel/default` et `templates/laravel/custom`
        - `<nom de fichier>` : Le nom du template, sensible à la case, à configurer. Obligatoire pour tout template à utiliser.
            ...
            - `PROJECTS_DIR` : (optionel) Répertoire dans lequel écrire le fichier issu du template, écrase l'option `PROJECT_TYPE.PROJECT_TYPE.project_docker_files_dir` si défini. Pour la racine du projet, utiliser `/`, `.` ou `./`
