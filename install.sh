...
INSTALL_DIR="/opt/${PROGRAM_COMMAND_NAME}"
source "${MAIN_SCRIPT_DIR}/global.var.sh"

source "${MAIN_SCRIPT_DIR}/fct/terminal-tools.fct.sh"

[ -n "${PROGRAM_COMMAND_NAME}" ] || eout "Impossible de trouver le nom de commande. Ajouter PROGRAM_COMMAND_NAME=\"conteur\" à ${MAIN_SCRIPT_DIR}/global.var.sh"
lout "-- Installation de ${PROGRAM_COMMAND_NAME} --"
...
if ask_yn "Installer ${PROGRAM_COMMAND_NAME} de manière globale ?\n\tOui\t: ${INSTALL_DIR}\n\tNon\t: ${MAIN_SCRIPT_DIR}\n\tControl+C pour annuler\n\t"; then
...
BIN_LINK="${BIN_DIR}/${PROGRAM_COMMAND_NAME}"
...
lout "Commande utilisable : ${PROGRAM_COMMAND_NAME}"